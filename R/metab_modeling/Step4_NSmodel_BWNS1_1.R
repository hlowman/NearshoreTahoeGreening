...
library(here)
## source
source("Metabolism_Loria_Scordo/stan_utility.R")
...
# read data in
data <- read_rdump("Metabolism_Loria_Scordo/BWNS1_2021_sonde_list.R")

# set reference temperature
data$temp_ref <- 18.10146
...
model_path <- paste0("Metabolism_Loria_Scordo/",model)

# set specifications for stan
chains <- 3 # was 6
iter <-10000 #was 1000
warmup <- 5000 #was 1000
adapt_delta <- 0.99
max_treedepth <- 15
thin <- 1
...
stanfit <- sampling(sm, data = data, chains = chains, 
                    cores = chains, iter = iter, warmup = min(iter*0.5,warmup),
